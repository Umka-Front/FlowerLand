...
import { getjQuery, typeCheckConfig, getUID } from './mdb/util/index';
...
const EVENT_SELECT = `select.${DATA_KEY}`;
const EVENT_VALUE_CHANGE = `change.${DATA_KEY}`;
const EVENT_FETCH_ERROR = `fetch-error.${DATA_KEY}`;
...
class Mention {
  ...
  _getConfig(config) {
    ...
    const HTMLDataList = this._element
      ? SelectorEngine.findOne(`[data-target=${this._element.id}]`)
      : null;
  ...
  _openMention() {
    ...
    this._popper = new Popper(this._element, this._mentionContainer, {
  ...
  _handleSelect() {
    ...
    EventHandler.trigger(this._element, EVENT_SELECT, { value: itemValue });
...
SelectorEngine.find(SELECTOR_MENTION).forEach((el) => {
  let instance = Mention.getInstance(el);
  const id = el.id;
  if (!instance && SelectorEngine.findOne(`.mention-data-items[data-target="${id}"]`)) {
